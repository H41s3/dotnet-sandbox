...
namespace Vector
{
    ...
    // Student now implements IComparable<Student> so it knows how to compare itself
    // to another Student. The natural/default ordering is by ascending ID.
    public class Student : IComparable<Student>
    {
        ...
        // IComparable<Student> contract: CompareTo returns
        //   negative  -> this student should come BEFORE 'another'
        //   zero      -> they are considered equal
        //   positive  -> this student should come AFTER 'another'
    ...
    // Comparator class: sorts students by ascending ID.
    // Identical outcome to Student.CompareTo, but as an external judge.
    ...
    // Comparator class: sorts students by DESCENDING name (Z → A),
    // breaking ties with DESCENDING ID (higher ID first).
    public class DescendingNameDescendingIdComparer : IComparer<Student>
    {
        public int Compare(Student A, Student B)
        {
            ...
            // Names are equal — break tie by descending ID (higher ID first).
            return B.Id - A.Id;
        }
    }
